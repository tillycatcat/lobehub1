...
import { AGENT_DOCUMENT_INJECTION_POSITIONS, MessagesEngine } from '@lobechat/context-engine';
  ...
  if (resolvedAgentId) {
    try {
      ...
      agentDocuments = documents.map((doc) => ({
        ...
        loadPosition: normalizeDocumentPosition(doc.policy?.context?.position || doc.policyLoadPosition),
