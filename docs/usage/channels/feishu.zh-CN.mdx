  ...
  <Tabs items={['飞书', 'Lark（国际版）']}>
    ...
    <Tab>
      ...
      <Callout type={'warning'}>
        Lark（国际版）不支持部分飞书特有的权限码（如 `aily:*`、`corehr:*`、`cardkit:*`）。使用飞书的 JSON 在 Lark 上批量导入可能会失败。
      </Callout>
    </Tab>
  </Tabs>
...
<Steps>
  ...
  - **验证令牌** — 用于验证 webhook 事件是否来自飞书 / Lark
  - **加密密钥** — 用于解密飞书 / Lark 的加密事件负载

  > 这两个值可以在飞书 / Lark 开发者门户的 **事件订阅** → **加密策略** 中找到（位于页面顶部）。如果您还没有打开过事件订阅页面，可以先留空，在完成第四步后再回来填写。
...
<Steps>
  ...
  在事件订阅页面顶部的 **加密策略** 区域，您可以看到：

  - **Verification Token（验证令牌）** — 复制此值
  - **Encrypt Key（加密密钥）** — 复制此值

  返回 LobeHub，将这些值填入对应的字段中（如果之前留空了），然后重新保存配置。
