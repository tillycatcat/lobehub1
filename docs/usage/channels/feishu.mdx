...
<Steps>
  ...
  <Tabs items={['Feishu (飞书)', 'Lark (International)']}>
    <Tab>
      ```json
      {
        "scopes": {
    ...
    <Tab>
      ...
      <Callout type={'warning'}>
        Lark (international) does not support some Feishu-specific scopes (e.g. `aily:*`, `corehr:*`, `cardkit:*`). Using the Feishu JSON on Lark may cause the batch import to fail.
      </Callout>
    </Tab>
  </Tabs>
...
<Steps>
  ...
  - **Encrypt Key** — Used to decrypt encrypted event payloads from Feishu/Lark

  > You can find these values in the Feishu/Lark Developer Portal under **Event Subscription** → **Encryption Strategy** at the top of the page. If you haven't opened the Event Subscription page yet, you can leave these blank for now and fill them in after Step 4.
...
<Steps>
  ...
  ### Copy Verification Token and Encrypt Key (Optional)

  At the top of the Event Subscription page, under **Encryption Strategy**, you will see:

  - **Verification Token** — Copy this value
  - **Encrypt Key** — Copy this value

  Go back to LobeHub and paste them into the corresponding fields if you left them blank earlier, then save again.
...
| **Verification Token**     | No       | Verifies webhook event source                                        |
